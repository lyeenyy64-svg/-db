...
// ─── 채무자 매핑 ────────────────────────────────────────────────────────────
function findDebtor(code, name) {
  if (code && code !== "코드없음") {
    const r = db.prepare("SELECT id, name FROM debtors WHERE hub_code = ?").get(code);
    if (r) return r;
  }
  const baseName = name.replace(/\(.*?\)/g, "").trim();
  if (!baseName) return null;
  const byExact = db.prepare("SELECT id, name FROM debtors WHERE name = ?").get(baseName);
  if (byExact) return byExact;
  const byLike = db.prepare("SELECT id, name FROM debtors WHERE name LIKE ? LIMIT 1").get(`%${baseName}%`);
  return byLike || null;
}
...
const doImport = db.transaction(() => {
  ...
  for (const row of excelData) {
    const debtor = findDebtor(row.code, row.name);
    if (!debtor) {
      stats.unmatched++;
      ...
      if (!sched) {
        ...
        db.prepare(`UPDATE installment_schedules
            SET due_date=?, status=?, scheduled_amount=CASE WHEN ?> 0 THEN ? ELSE scheduled_amount END,
                paid_amount=CASE WHEN ?='완납' AND ?>0 THEN ? ELSE paid_amount END, memo=COALESCE(memo,?)
            WHERE id=?`)
